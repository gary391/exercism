...
def resistor_label(colors):
    """
    colors example:  [sig1, sig2, multiplier, tolerance]
    """
    if len(colors) == 1:
    ...
    *sig_colors, multiplier_color, tolerance_color = colors
    sig_value = 0
    for c in sig_colors:
        sig_value = sig_value * 10 + COLOR_TO_VALUE.index(c)
    multiplier_color = COLOR_TO_VALUE.index(multiplier_color)
    tolerance = TOLERANCE[tolerance_color]
    value = sig_value * 10 ** int(multiplier_color)
